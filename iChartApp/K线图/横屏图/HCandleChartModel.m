...
@implementation HCandleChartModel
...
-(void)drawSerie:(HChart *)chart serie:(NSMutableDictionary *)serie{    
    ...
    for(int i=chart.rangeFrom;i<chart.rangeTo;i++){
        ...
        if(i == chart.selectedIndex && chart.selectedIndex < data.count && [data objectAtIndex:chart.selectedIndex]!=nil){
            ...
            //指标线颜色
            CGContextSetStrokeColorWithColor(context, [[UIColor alloc] initWithRed:111/255.0f green:234/255.0f blue:52/255.0f alpha:1.0].CGColor);
            CGContextMoveToPoint(context,LINE_POINT,iyo);
            CGContextAddLineToPoint(context,sec.frame.size.width+sec.frame.origin.x,iyo);            
            CGContextStrokePath(context);
        }
